...
- (NSArray *)readImageData:(NSString *)filePath
{
    ...
    if (file) {
        ...
        int n_cols=0;
        fread((char*)&magic_number, sizeof(magic_number), 1, file);
        magic_number= [self reverseInt:magic_number];
        fread((char*)&number_of_images, sizeof(number_of_images), 1, file);
        number_of_images= [self reverseInt:number_of_images];
        fread((char*)&n_rows, sizeof(n_rows), 1, file);
        n_rows= [self reverseInt:n_rows];
        fread((char*)&n_cols, sizeof(n_cols), 1, file);
        n_cols= [self reverseInt:n_cols];
        output = [NSMutableArray arrayWithCapacity:number_of_images];
        for(int i=0;i<number_of_images;++i)
        {
            NSMutableArray *temp = [NSMutableArray arrayWithCapacity:n_rows*n_cols];
            output[i] = temp;
            for(int r=0;r<n_rows;++r)
            {
                for(int c=0;c<n_cols;++c)
                {
                    unsigned char temp=0;
                    fread((char*)&temp, sizeof(temp), 1, file);
                    output[i][(n_rows*r)+c]= [NSNumber numberWithDouble:(double)temp];
                }
            }
        }
    }
...
- (NSArray *)readLabelData:(NSString *)filePath
{
    ...
    if (file) {
        ...
        magic_number= [self reverseInt:magic_number];
        fread((char*)&number_of_items, sizeof(number_of_items), 1, file);
        number_of_items= [self reverseInt:number_of_items];
        output = [NSMutableArray arrayWithCapacity:number_of_items];
        for(int i=0;i<number_of_items;++i)
        {
            unsigned char temp=0;
            fread((char*)&temp, sizeof(temp), 1, file);
            output[i]= [NSNumber numberWithInt:(int)temp];
        }
    }
    fclose(file);
    return output;
